  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg border border-slate-200 w-full max-w-lg p-6 shadow-lg">
        <h2 className="text-base font-semibold text-slate-900 mb-1">Test delivery</h2>
        <p className="text-xs text-slate-400 mb-4 break-all">{endpointLabel}</p>

        {loading ? (
          <p className="text-sm text-slate-500 py-6 text-center">Sending test event...</p>
        ) : result ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              {result.status_code != null && (
                <span className="text-xs text-slate-500">HTTP {result.status_code}</span>
              )}
            ...
            {result.error && (
              <div>
                <div className="text-xs font-medium text-slate-600 mb-1">Error</div>
            ...
            {result.response_snippet && (
              <div>
                <div className="text-xs font-medium text-slate-600 mb-1">Receiver response</div>
