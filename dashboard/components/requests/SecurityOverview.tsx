  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <KpiCard label={tc("decision.sanitized")} value={<Count n={s.sanitize_count} />} valueColor="#d97706" accent="#d97706" sub={formatRate(s.sanitize_rate)} />
        <KpiCard label={tc("decision.blocked")} value={<Count n={s.block_count} />}    valueColor="#dc2626" accent="#dc2626" sub={formatRate(s.block_rate)} />
        <KpiCard label={t("block_rate")}         value={formatRate(s.block_rate)}       accent="#dc2626" />
        <KpiCard label={t("average_risk")}       value={formatScore(s.avg_risk)}        valueColor={riskColor(s.avg_risk)} accent={riskColor(s.avg_risk)} />
      </div>
    </div>
  )
}
