...
- **Guardrails** — enforce data protection policies (PII — deterministic)

**Guardrail-first enforcement**

Guardrail decisions override detection decisions unconditionally. PII scores never contribute to the detection risk score.

**Threshold decoupling**

Detection thresholds (`thresholds.block`, `thresholds.sanitize`) and guardrail thresholds (`guardrails.pii.block_threshold`, `guardrails.pii.sanitize_threshold`) are always independent. Changing one never affects the other.
...
if guardrail_triggered:        return "PII_GUARDRAIL_BLOCK/SANITIZE"
...
Input
  ...
  │       full mode AND pre-score >= llm_trigger
  ...
  │
  ├─► [6] Guardrail Evaluation (independent)
  │       Uses guardrails.pii.block_threshold    ≠ thresholds.block
  │       Uses guardrails.pii.sanitize_threshold ≠ thresholds.sanitize
  ...
  │       2. guardrail triggered    → PII_GUARDRAIL_BLOCK/SANITIZE
  │       3. max detector score > 0 → RULE/ML/LLM_DETECTOR
  │       4. all scores = 0.0      → NO_THREAT_DETECTED
...
def compute_primary_reason(
    ...
    # Priority 2: guardrail override
    if guardrail_triggered:
        if pii_score >= pii_block_threshold:
            return "PII_GUARDRAIL_BLOCK"
        return "PII_GUARDRAIL_SANITIZE"

    # Priority 3: dominant detection layer
    scores = {
    ...
    # Priority 4: genuinely clean input
    # Only reachable when:
...
| `PII_GUARDRAIL_BLOCK` | PII >= pii_block_threshold | tiered 0.90–0.95 | Data handling review |
| `PII_GUARDRAIL_SANITIZE` | PII >= pii_sanitize_threshold | tiered 0.70–0.84 | Monitor sanitisation |
...
```python
if pii_score >= pii_block_threshold:
    confidence = 0.90 + (min(pii_score, 1.0) - pii_block_threshold) * 0.05
    confidence = min(confidence, 0.95)

elif pii_score >= pii_sanitize_threshold:
    confidence = 0.70 + (pii_score - pii_sanitize_threshold) * 0.20
...
| `LOW` | 0.0 – 0.4 | Human review or system failure |
