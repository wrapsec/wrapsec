...
| `RATE_LIMIT_ENABLED` | `true` | Master switch for the GLOBAL rate-limit middleware (`rate_limit.py:91`): the per-source and per-key buckets on every rate-limited path. Setting it `false` removes both. It does NOT reach the login limit, the admin-write limit or the audit-export limit, which are separate layers and keep running. Intended for local development only |
| `WEBHOOK_BLOCK_PRIVATE_EGRESS` | `true` | Resolve-time block on a webhook destination that lands on a private, loopback, link-local or cloud-metadata address. **Setting it `false` returns before every one of those checks, the metadata-host block included**, so `169.254.169.254` becomes a deliverable destination and an admin-credentialed caller could aim a webhook at the instance metadata service. Prefer `WEBHOOK_EGRESS_ALLOWLIST` for an on-prem receiver rather than turning the control off |
| `WEBHOOK_EGRESS_ALLOWLIST` | `""` | Comma-separated hosts or CIDRs exempt from the private-address block - the narrow way to reach an internal SIEM. An allowlisted host is also exempt from the https requirement, so list only receivers you control |
| `WEBHOOK_REQUIRE_HTTPS` | `true` | Require https for webhook destinations. Checked BEFORE the private-egress opt-out, so it still applies when that is disabled. With it `false`, security events are delivered in plaintext |
...
| `WEBHOOK_DELIVERY_WORKER_ENABLED` | `true` | Run the outbound delivery worker in this process. Disable it on a node that should serve traffic without draining the queue. **At least one node must run it.** With it off everywhere, `BLOCK` and `SANITIZE` events stop reaching every configured destination while the dashboard and audit log look entirely normal - the security-event pipeline goes silent without an error anywhere |
...
2. **Encrypted provider credentials become unreadable** - department and application proxy provider API keys are stored encrypted using `SECRET_KEY` (`api_key_enc` in `policy_override`). As of v1.1.0 storage uses envelope encryption (B3): each secret is protected by a random per-record DEK wrapped by a KEK derived from `SECRET_KEY`. Rotating `SECRET_KEY` still breaks the KEK unwrap, so decryption fails and any proxy call using a stored provider credential returns a 500 error. To restore proxy functionality: go to the dashboard -> the affected Department or Application -> Proxy Provider -> re-enter the provider API key -> Save. This re-encrypts it with the new `SECRET_KEY`. When KEK-in-KMS lands (v1.5+), the KEK will rotate independently of `SECRET_KEY` and re-wrapping DEKs will not require re-entering any provider key.
