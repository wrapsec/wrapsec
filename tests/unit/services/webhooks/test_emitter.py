...
The emitter is scheduled via FastAPI BackgroundTasks, so it runs AFTER
the scan response is on the wire. These tests pin the invariants that
matter for that placement:
  ...
  7. Errors in the DB lookup or the enqueue are swallowed -- the emitter
     is never allowed to raise into the caller (even though the caller
     is BackgroundTasks; raising here would only pollute logs).
  8. emit_from_audit_background opens its own session, gets its own
     Redis handle, and swallows exceptions -- it fires after the
     response is sent, so raising serves no purpose.
"""
...
# ─── emit_from_audit_background: BackgroundTasks entry point ─────────────

@pytest.mark.asyncio
async def test_background_opens_session_and_calls_emit_from_audit():
    """The BackgroundTasks entry point owns its own session (the request-
    scoped db is closed by the time the task fires) and delegates to
    emit_from_audit with the same audit dict it was handed."""
    audit_data = _audit()
    ...
    with patch.object(emitter, "AsyncSessionFactory", return_value=session_ctx), \
         patch.object(emitter, "get_redis") as get_r, \
         patch.object(emitter, "emit_from_audit", new=AsyncMock(return_value=1)) as emit:

        redis_stub = MagicMock()
        get_r.return_value = redis_stub

        await emit_from_audit_background(audit_data)

        emit.assert_awaited_once()
        call = emit.await_args
        assert call.kwargs["db"] is session_mock
        assert call.kwargs["redis"] is redis_stub
        assert call.kwargs["audit_data"] is audit_data
        session_ctx.__aexit__.assert_awaited_once()


@pytest.mark.asyncio
async def test_background_swallows_any_exception():
    """Background task fires after the response is sent -- raising here
    would only pollute logs. Emitter, session-open, or redis-fetch errors
    must all be swallowed."""
    with patch.object(emitter, "AsyncSessionFactory", side_effect=RuntimeError("db unavailable")):
        # Must not raise:
        await emit_from_audit_background(_audit())
