...
The emitter calls this once per BLOCK/SANITIZE decision (via the
BackgroundTasks queue, not the response path), so these tests pin the
filter predicates that make the fanout correct:

  1. Tenant isolation -- endpoints belonging to a different tenant are
     never returned. (Same class of bug guarded by
     test_cross_tenant_isolation.py for other repos.)
